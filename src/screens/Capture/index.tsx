...
export default function Capture({ transcriptLines, topics, sentimentPts, onRecordingChange }: CaptureProps = {}) {
	const { role } = useRole();
	const { user } = useUser();
	...
	const controlsContent = (
		...
		{phase === 'review' && (
			<button
			onClick={() => { setTranscript(''); toogleRecording; }}
			style={{
		...
		{phase === 'idle' || phase === 'review' ? (
			<button
			className="rec-btn"
			onClick={phase === 'idle' ? toggleRecording : handleSave}
		...
		<button className="rec-btn" onClick={toogleRecording}>
		<IconStop />
		</button>
		) : (
