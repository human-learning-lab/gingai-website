  ...
  function handleStop() {
    ...
    setTimeout(() => {
      ...
      if (next >= STEPS.length) {
        ...
        setStep(next);
        setRecTime(60 + Math.floor(Math.random() * 60));
        setPhase('recording');
        timerRef.current = setInterval(() => setRecTime(prev => (prev > 0 ? prev - 1 : 0)), 1000);
      }
      setVisible(true);
      ...
      <div className="phone">
        ...
        <div className="cap-hdr">
          ...
          <div className="why-bar">
            {STEPS.map((_, i) => (
              <div key={i} className={`why-seg${i < doneCount ? ' done' : i === step ? ' now' : ''}`} />
            ))}
          </div>
        ...
        {phase === 'done' ? (
          ...
          <div className="cap-convo" style={{ opacity: visible ? 1 : 0, transition: 'opacity 0.35s ease' }}>
            {current.prevAnswer && <SailorR text={current.prevAnswer} faded />}
            {phase === 'processing' ? (
              <div className="ai-q">
                <GingAIAvatar />
                <div className="ai-q-bub" style={{ color: 'var(--text3)', fontStyle: 'italic' }}>Analysing your response…</div>
              </div>
            ) : (
              <AiQ why={current.why ?? undefined} whyColor={current.whyColor ?? undefined} recording text={current.q} />
            )}
          </div>
        )}
          ...
          {phase === 'processing' && (
            <div style={{ height: 28, display: 'flex', alignItems: 'center', gap: 4 }}>
              {Array.from({ length: 3 }).map((_, i) => (
                <div key={i} style={{ width: 6, height: 6, borderRadius: '50%', background: 'var(--green)', animation: `pdot 1.2s ease-in-out ${i * 0.2}s infinite` }} />
              ))}
            </div>
          )}
          {phase === 'done' ? (
    ...
    <div className="cap-readonly">
      <div className="cap-ro-header">
        <div style={{ display: 'flex', alignItems: 'center', gap: 10, marginBottom: 6 }}>
          <div className="eyebrow" style={{ marginBottom: 0 }}>Capture · Post R5/R6/R7</div>
