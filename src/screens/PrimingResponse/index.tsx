...
function Page({ runId, sailor, transcriptLines, onRecordingChange }: 
{
  ...
  useEffect(() => {
	  async function getResp(){
		...
		const mirrored = await fetch(
			`/api/question-set?runId=${encodeURIComponent(runId)}&sailor=${encodeURIComponent(sailor.firstName)}&kind=priming`,
		);
		if (mirrored.ok) {
	  	...
	  	const res = await fetch(`/api/responses/${runId}?kind=priming&sailor=${encodeURIComponent(sailor.firstName)}`);
  ...
  async function submitAll(answers: string[]) {
    ...
    try {
      ...
      void pruneClips(runId, "priming", sailor.firstName, [...filedPaths.current])
        .catch(() => undefined);
    } catch (err) {
