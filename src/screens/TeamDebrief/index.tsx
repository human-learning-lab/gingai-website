          ...
          <div className="db-side">
            <div className="dbs-sec">
              ...
              <div className="dbd-graph-card">
                <div className="dbd-placeholder-note">
                  {recording ? 'Waiting for data…' : 'Start recording to see talking time'}
                </div>
              </div>
            </div>
            ...
            <div className="dbs-sec">
              ...
              <div className="dbd-graph-card">
                {activeSentiment.length > 1 ? (
                  <>
                    <svg viewBox={`0 0 ${SVG_W} ${SVG_H}`} className="dbd-svg" preserveAspectRatio="none">
                      <defs>
                      ...
                      <polygon points={`0,${SVG_H} ${sentPts} ${SVG_W},${SVG_H}`} fill="url(#sent-grad)" />
                      <polyline points={sentPts} fill="none" stroke="var(--green)" strokeWidth="1.5" strokeLinejoin="round" strokeLinecap="round" />
                    </svg>
                  ...
                  <div className="dbd-placeholder-note">
                    {recording ? 'Waiting for data…' : 'Start recording to see sentiment'}
                  </div>
                )}
              </div>
            </div>
              ...
              <div className="dbd-graph-card">
                <div className="dbd-topics">
                  {topics && topics.length > 0
                    ? topics.map(t => <span key={t} className="dbd-topic-tag">{t}</span>)
                    : <span className="dbd-placeholder-note">
                        {recording ? 'Waiting for topics…' : 'Topics appear as recording runs'}
                      </span>
                  }
                </div>
                {topics && topics.length > 0 && (
