...
/**
 * POST /api/priming-artifacts  { runId, teamPicture?, briefingGoals?, distilled? }
 *
...
export async function POST(req: NextRequest) {
  try {
    const { runId, teamPicture, briefingGoals, distilled } = await req.json();
    if (!runId) {
      return NextResponse.json({ error: 'runId is required' }, { status: 400 });
    }
    if (teamPicture === undefined && briefingGoals === undefined && distilled === undefined) {
      return NextResponse.json({ error: 'Nothing to save' }, { status: 400 });
    }

    const saved = await savePrimingArtifacts({ runId, teamPicture, briefingGoals, distilled });
